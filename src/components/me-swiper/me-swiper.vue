<!--
	...
	interval：每两次隔多久滚动一次
	duration：每次滚动一页需要多久时间
	event:
		prevSlide：上一张
		nextSlide：下一张
		slideTo(num)：跳转到第几张。参数：num
-->

<template>
	<section class="wh_content" @touchmove="fn">
		<me-icon name="left" class="pre" @click="prevSlide" v-if="swiperBtn"></me-icon>
		<me-icon name="right" class="next" @click="nextSlide" v-if="swiperBtn"></me-icon>
